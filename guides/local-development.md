...
For deployed DuckLake catalogs, server-side DuckLake maintenance paths such as `read_blob()` orphan discovery require a Worker R2 bucket binding for the DuckLake `DATA_PATH` bucket. Trusted-client data leases also require that binding so quacklake can validate the catalog `DATA_PATH` bucket before minting temporary credentials. See [Quack, DuckLake, And R2 Guide](./quack-ducklake.md) for storage secrets, Worker R2 binding setup, trusted-client leases, `DATA_PATH` behavior, bucket listing, diagnostics, and file inventory endpoints.
